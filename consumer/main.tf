...
# -------------------------------------------------------------------------------------
# Create NSI Consumer Resources
# -------------------------------------------------------------------------------------

# -------------------------------------------------------------------------------------
#  If mirroring_mode = false, create intercept deployment.
# -------------------------------------------------------------------------------------
...
resource "google_network_security_intercept_endpoint_group" "main" {
  ...
  intercept_endpoint_group_id = "${local.prefix}epg"
  location                    = "global"
...
resource "google_network_security_intercept_endpoint_group_association" "main" {
  provider                                = google-beta
  count                                   = var.mirroring_mode ? 0 : 1
  intercept_endpoint_group_association_id = "${local.prefix}epg-assoc"
  location                                = "global"
  network                                 = google_compute_network.main.id
...
# -------------------------------------------------------------------------------------
#  If mirroring_mode = true, create mirroring deployment.
# -------------------------------------------------------------------------------------
...
resource "google_network_security_mirroring_endpoint_group" "main" {
  ...
  mirroring_endpoint_group_id = "${local.prefix}epg"
  location                    = "global"
  mirroring_deployment_group  = var.producer_dg
}

// Create mirroring endpoint association
resource "google_network_security_mirroring_endpoint_group_association" "main" {
  provider                                = google-beta
  count                                   = var.mirroring_mode ? 1 : 0
  mirroring_endpoint_group_association_id = "${local.prefix}epg-assoc"
  location                                = "global"
  network                                 = google_compute_network.main.id
...
resource "google_network_security_security_profile" "main" {
  ...
  description = "NSI Security Profile"
  type        = var.mirroring_mode ? "CUSTOM_MIRRORING" : "CUSTOM_INTERCEPT"

  dynamic "custom_intercept_profile" {
    for_each = var.mirroring_mode ? [] : [1]
    content {
      intercept_endpoint_group = google_network_security_intercept_endpoint_group.main[0].id
    }
  }
...
resource "google_network_security_security_profile_group" "main" {
  provider                 = google-beta
  name                     = "${local.prefix}spg"
  parent                   = "organizations/${data.google_project.current.org_id}"
  description              = "NSI Security Profile Group"
  custom_intercept_profile = var.mirroring_mode ? null : google_network_security_security_profile.main.id
  custom_mirroring_profile = var.mirroring_mode ? google_network_security_security_profile.main.id : null
}

resource "google_compute_network_firewall_policy" "main" {
  name        = "${local.prefix}consumer-policy"
  description = "NSI Consumer Firewall Policy"
}
